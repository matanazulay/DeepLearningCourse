...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for i in xrange(T):
    xs = x[:, i, :]
    hs[i], _ = rnn_step_forward(xs, hs[i - 1], Wx, Wh, b)
    h[:, i, :] = hs[i]
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  ##############################################################################
  next_c = f * prev_c + i * g
  next_c_grad = np.tanh(next_c)
  next_h = o * next_c_grad
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  ddprev_c = dnext_c + (1 - next_c_grad**2) * dnext_h * o
  df = ddprev_c * prev_c
  dprev_c = ddprev_c * f
 
  do = dnext_h * next_c_grad * o * (1 - o)
  df = ddprev_c * prev_c * (1 - f) * f
  di = ddprev_c * g * i * (1 - i)
  dg = ddprev_c * i * (1 - g**2)
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  caches = []
  for t in range(T):
      h0, c, cache_forward = lstm_step_forward(x[:, t, :], h0, c, Wx, Wh, b)
      h[:, t, :] = h0
      caches.append(cache_forward)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  cache = x, h0, Wx, Wh, c, caches
  return h, cache
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(range(T)):
    cache_iterartion = caches[t]
    dx_temp, dh0, dc, dWx_temp, dWh_temp, db_temp = lstm_step_backward(dh[:, t, :] + dh0, dc, cache_iterartion)
    dWh += dWh_temp
    dWx += dWx_temp
    db += db_temp
    dx[:, t, :] = dx_temp
